#!/usr/bin/env python3
"""Create a deterministic tar archive of the pinned addendum inputs."""
...
MANIFEST = ROOT / "input_sources.tsv"
DEFAULT_OUTPUT = ROOT / "comparative-addendum-inputs-v1.tar"
...
def create_archive(rows: list[dict[str, str]], output: Path) -> None:
    ...
    try:
        with temporary:
            with tarfile.open(fileobj=temporary, mode="w", format=tarfile.USTAR_FORMAT) as archive:
                for row in rows:
                    path = ROOT / row["relative_path"]
                    info = tarfile.TarInfo(name=row["relative_path"])
